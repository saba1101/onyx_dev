...
import { PlusIcon, SlidersIcon, PencilIcon, XIcon, TrashIcon, ChevronIcon, LockIcon, GlobeIcon } from "~/components/ui/icons"
...
export default function BoardPage() {
  ...
  const [project,  set_project]  = useState<WProject | null>(null)
  ...
  const is_owner    = !!(project && user && project.owner_id === user.id)
  const permissions = usePermissionsStore()
  const can_manage_columns = is_root || is_owner || permissions.manage_columns

  // ── Load ──────────────────────────────────────────────────────────────────

  const load = useCallback(async () => {
    if (!project_id) return
    const [pr_res, s_res, t_res, p_res] = await Promise.all([
      api.projects.get(project_id),
      api.statuses.list(project_id),
      api.tasks.list(project_id),
      api.profiles.list(),
    ])
    if (pr_res.error || !pr_res.data) { set_not_found(true); set_loading(false); return }
    ...
    set_project(pr_res.data as WProject)
  ...
  useEffect(() => {
    if (!project_id) return
    const ch = supabase
      .channel(`workspace_rt_${project_id}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "workspace_tasks", filter: `project_id=eq.${project_id}` }, () => {
        api.tasks.list(project_id).then(({ data }) => {
          if (data) set_tasks(data as WTask[])
        })
      })
      .on("postgres_changes", { event: "*", schema: "public", table: "workspace_statuses", filter: `project_id=eq.${project_id}` }, () => {
        api.statuses.list(project_id).then(({ data }) => {
  ...
  if (!user) return null
  if (not_found) return <Navigate to="/workspace" replace />
  ...
  const project_color = project ? STATUS_COLORS[project.color] : null

  return (
    <div className="flex h-full overflow-hidden" onDragEnd={on_drag_end}>
      <SideRail />
      ...
      <main className="flex flex-1 flex-col overflow-hidden pt-14 lg:pt-0 min-h-0">
        ...
        >
          {/* Breadcrumb row */}
          <div className="flex items-center gap-2 px-4 pt-3 lg:px-6">
            <Link to="/workspace" className="inline-flex items-center gap-1 text-[11px] font-medium text-muted transition-colors hover:text-ink">
          ...
          <div className="flex items-center justify-between gap-3 px-4 py-3 lg:gap-4 lg:px-6">
            <div className="flex min-w-0 items-center gap-3">
              <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${project_color?.badge ?? "bg-flag-red/10 text-flag-red"}`}>
                <KanbanIcon />
              </span>
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <h1 className="truncate text-sm font-bold text-ink">{project?.name ?? "Workspace"}</h1>
                  <span className="inline-flex shrink-0 items-center gap-1 rounded-md border border-line px-1.5 py-0.5 text-[9px] font-medium uppercase tracking-wide text-muted">
            ...
            {active_tab === "board" && (
              <div className="flex shrink-0 items-center gap-2">
                ...
                <button
                  type="button"
                  onClick={() => can_manage_columns ? set_col_panel_open(true) : set_col_perm_error(true)}
        ...
        {active_tab === "analytics" ? (
          ...
          <EmptyBoard on_manage={() => can_manage_columns ? set_col_panel_open(true) : set_col_perm_error(true)} />
        ) : (
