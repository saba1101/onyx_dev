...
export const get_stored_token = async (user_id: string): Promise<string | null> => {
...
export const save_github_token = (user_id: string, token: string, username: string) =>
  supabase
    .from("profiles")
...
export const connect_github = (already_linked = false) =>
  already_linked
    ? supabase.auth.signInWithOAuth({
        provider: "github",
        options: {
          scopes: "read:user repo",
          redirectTo: `${window.location.origin}/git`,
        },
      })
...
export const gh_repos    = (t: string)               => gh<GHRepo[]>("/user/repos?per_page=100&sort=updated&affiliation=owner,collaborator,organization_member", t)
export const gh_langs    = (t: string, full: string) => gh<LangMap>(`/repos/${full}/languages`, t)
export const gh_activity = (t: string, full: string) => gh<GHCommitWeek[]>(`/repos/${full}/stats/commit_activity`, t)
export const gh_events   = (t: string, login: string) => gh<GHEvent[]>(`/users/${login}/events?per_page=50`, t)
...
export const commit_heatmap = (week_sets: GHCommitWeek[][]): number[] => {
  ...
  for (const weeks of week_sets) {
    ...
    weeks.slice(-52).forEach((w, wi) =>
      w.days.forEach((c, di) => { grid[wi * 7 + di] += c }),
    )
  }
  return grid
}

export const weekly_totals = (week_sets: GHCommitWeek[][]): { week: string; commits: number }[] => {
  const agg = new Map<number, number>()
  for (const weeks of week_sets) {
    if (!Array.isArray(weeks)) continue
    weeks.slice(-52).forEach(w => agg.set(w.week, (agg.get(w.week) ?? 0) + w.total))
  }
  return [...agg.entries()]
    .sort(([a], [b]) => a - b)
    .map(([ts, commits]) => ({
      week: new Date(ts * 1000).toLocaleDateString(undefined, { month: "short", day: "numeric" }),
      commits,
    }))
}
